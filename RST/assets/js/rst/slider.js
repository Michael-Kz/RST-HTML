class rstSlider {
    ...
    constructor(el, config = {}, prefix = 'rst-slider__') {
  
      this.#state = {
        prefix: prefix, // префикс для классов
        el: el, // элемент который нужно активировать как rstSlider
      ...
      this.#config = {
        loop: false, autoplay: false, interval: 7000, refresh: true, swipe: true, ...config
      };
    ...
    static getOrCreateInstance(target, config = {}, prefix = 'rst-slider__') {
      try {
    ...
    static createInstances() {
      document.querySelectorAll('[data-slider="rst-slider"]').forEach((el) => {
    ...
    #attachEvents() {
      this.#state.events = {
        ...
        'rst-slider__transition-start': [this.#state.elItems, this.#onTransitionStart.bind(this), this.#config.loop],
    ...
    #move() {
      ...
      this.#state.elItems.dispatchEvent(new CustomEvent('rst-slider__transition-start', {
        bubbles: true
      }));
    }
